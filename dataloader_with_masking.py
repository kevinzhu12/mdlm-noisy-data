...
def apply_sequence_masking(dataset, tokenizer, clean_ratio=0.1, token_keep_prob=0.5, seed=None):
  """Apply masking to a dataset for training discrete diffusion models without clean data.
  
  Args:
    dataset: HuggingFace dataset with 'input_ids' column containing tokenized sequences
    tokenizer: Tokenizer with mask_token attribute
    clean_ratio: Fraction of sequences to keep clean (default: 0.1 for 10% clean)
    ...
    if isinstance(input_ids[0], list):
      ...
      for seq in input_ids:
        if random.random() < clean_ratio:
          ...
          # Apply masking to sequence
          masked_seq = []
          for token_id in seq:
            # Skip special tokens (BOS, EOS, PAD) from masking
            if (token_id == tokenizer.bos_token_id or 
                token_id == tokenizer.eos_token_id or 
                token_id == tokenizer.pad_token_id):
              masked_seq.append(token_id)
            else:
              # Flip coin to decide whether to mask this token
              if random.random() < token_keep_prob:
                masked_seq.append(token_id)  # Keep original token
              else:
                masked_seq.append(mask_token_id)  # Mask token
          masked_input_ids.append(masked_seq)
      example['input_ids'] = masked_input_ids
    else:
      # Single sequence
      if random.random() < clean_ratio:
        # Keep sequence clean - no changes needed
        pass
      else:
        # Apply masking to sequence
        masked_seq = []
  ...
  masked_dataset = dataset.map(
    mask_sequence,
    batched=True,
    desc='Applying sequence masking')
...
def get_dataset(
    ...
    apply_masking=False, clean_ratio=0.1, token_keep_prob=0.5, masking_seed=None):
  if wrap:
    filename = f'{dataset_name}_{mode}_bs{block_size}_wrapped'
  else:
    filename = f'{dataset_name}_{mode}_bs{block_size}_unwrapped'
  
  # Add masking parameters to filename if masking is applied
  if apply_masking:
    filename += f'_masked_clean{clean_ratio}_keep{token_keep_prob}'
  ...
  # Apply masking if requested (after grouping for wrapped datasets)
  if apply_masking:
...
def get_dataloaders(config, tokenizer, skip_train=False,
  ...
  if skip_train:
    ...
    train_set = get_dataset(
      ...
      token_keep_prob=token_keep_prob,
